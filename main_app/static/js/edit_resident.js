...
$('#editResidentForm').on('submit', function(e){
    ...
    $('#editResidentBtn').prop('disabled', true);
    e.preventDefault();
    console.log("1");
    ...
    formData.append('old_img_file_directory', $('#old_img_file_directory').val());
    formData.append('resident_id', $('#resident_id').val());
    ...
    $.ajax({
        ...
        error: function(data){
            $('#editResidentBtn').prop('disabled', false);
            Swal.fire({
                icon: 'error',
                title: 'Oops...',
              })
